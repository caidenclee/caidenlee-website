...
function updateScrambleImg(s) {
    ...
    if (!img) return;
    var on = localStorage.getItem('zblsDrawScramble') === 'on';
...
function fillResultInfo(r) {
    ...
    if (r != null) {
        var imgSrc = 'caseImage/ZBLS/level3/' + r["oll"] + '-' + r["coll"] + '-' + r["zbll"] + '.png';
        var s = "";
        ...
        if (incorrectKeys.length > 0) {
            ...
            retryBtn.onclick = function() {
                ...
                const cases = incorrectKeys.map(k => {
                    ...
                    return {
                        p: 1,
                        algs: zblsMap[c.oll][c.coll][c.zbll].algs,
                        desc: c.desc,
...
function saveSession() {
    ...
    try {
        const saved = JSON.parse(localStorage.getItem('zblsSessions') || '[]');
        saved.push(record);
        localStorage.setItem('zblsSessions', JSON.stringify(saved));
...
window.retryHistorySession = function(incorrectCases) {
    const cases = incorrectCases
        .filter(c => c && c.oll && c.coll && c.zbll
            && zblsMap[c.oll] && zblsMap[c.oll][c.coll] && zblsMap[c.oll][c.coll][c.zbll])
        .map(c => ({
            p: 1,
            algs: zblsMap[c.oll][c.coll][c.zbll].algs,
            desc: c.desc,
